...
class HandDetector:
        ...
        if self.capture.isOpened():
            ...
            while self.capture.isOpened():
                ...
                if ret is True:
                    ...
                    # TODO: capture frame -> get data to csv, further: mutlihands
    ...
    def draw_hand_overlay(self, frame, hand):
        ...
        """
        # if hand._detected:
        #     for finger_number, fingertip in enumerate(hand._fingertips):
        ...
        #                     (255, 255, 255),
        #                     1)
        #     for defect in hand._intertips:
        #         cv2.circle(frame, tuple(defect), 8, [211, 84, 0], -1)
        self.draw_contour_features(frame, hand._contour)
        ...
        if hand._detected or hand._tracked:
            cv2.drawContours(frame, [hand._contour], -1, (255, 255, 255), 2)

        points = np.array(hand._position_history)
        cv2.polylines(img=frame, pts=np.int32([points]), isClosed=False, color=hand._color)
        tail_length = 15
        ...
        if hand._center_of_mass is not None:
            # Draw center mass
            cv2.circle(frame, hand._center_of_mass, 7, [100, 0, 255], 2)
            cv2.putText(frame, 'Center', tuple(hand._center_of_mass), self.font, 0.5, (255, 255, 255), 1)

        hand_string = "hand %d %s: D=%s|T=%s|L=%s|F=%d" % (
        hand.id, str(hand._center_of_mass), str(hand._detected), str(hand._tracked), str(hand._confidence),
        hand._frame_count)
